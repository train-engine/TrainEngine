...
LoopDebugOverlay::LoopDebugOverlay(const sf::Font& font)
    : m_upsText("UPS: ", font, 15),
      ...
      m_samplingTime(sf::milliseconds(250)),
      m_recordedUps(0),
      m_recordedFps(0),
      m_updateCounter(0),
      m_drawCounter(0),
...
void LoopDebugOverlay::RecordUpdate(sf::Time lastUpdateTime)
{
    ...
    if (m_upsClock.getElapsedTime() >= m_samplingTime) // Only recalculate after a set amount of time
    {
...
void LoopDebugOverlay::RecordDraw(sf::Time lastDrawTime)
{
    ...
    if (m_fpsClock.getElapsedTime() >= m_samplingTime) // Only recalculate after a set amount of time
    {
