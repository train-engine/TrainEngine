...
public:
    ...
    void AddTile(TileType tileType, const sf::Vector2u& tileIndex, MapLayer layer, bool updateTextures = false) {m_map.AddTile(tileType, tileIndex, layer, updateTextures);}
    void AddTileRange(TileType tileType, const sf::Vector2u& tileIndex, const sf::Vector2u& range, MapLayer layer, bool updateTextures = false) {m_map.AddTileRange(tileType, tileIndex, range, layer, updateTextures);}
    void RemoveTile(const sf::Vector2u& tileIndex, MapLayer layer, bool updateTextures = false) {m_map.RemoveTile(tileIndex, layer, updateTextures);}
    void RemoveTileRange(const sf::Vector2u& tileIndex, const sf::Vector2u& range, MapLayer layer, bool updateTextures = false) {m_map.RemoveTileRange(tileIndex, range, layer, updateTextures);}
